...
- (void)viewDidLoad {
	...
	UIBarButtonItem * mgmtButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonSystemItemAction target:self action:@selector(cancelAction)];
	self.navigationItem.leftBarButtonItem = mgmtButtonItem;
	[mgmtButtonItem release];
	
	UIBarButtonItem * shareButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"发布" style:UIBarButtonSystemItemAction target:self action:@selector(shareAction)];
...
- (void)setupShareView {
    ...
    UITextView * shareView = [[UITextView alloc] initWithFrame:CGRectMake(10.0f, 10.0f, self.view.bounds.size.width - 20.0f, 120.0f)];
	[shareView setAutoresizingMask:UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleBottomMargin];
	...
	for (int i = 0; i< ([self.imageShareList count] > 5 ? 5 : [self.imageShareList count]); i++) {
		...
		loadButton.frame = CGRectMake(10.0f+ (buttonWidth + 12.5)*i, 140.0f, buttonWidth, buttonWidth);
        [loadButton setAutoresizingMask:UIViewAutoresizingFlexibleBottomMargin];
		loadButton.tag = i;
